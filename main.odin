...
components_to_include_in_project :: proc (leaves: ^[dynamic]zd.Leaf_Template) {
    ...
    zd.append_leaf (leaves, zd.Leaf_Template { name = "<b>= <i>init</i></b><br>p.ok = max_hp<br>p.wallet = 0<br>p.pwr = 4<br>ndays = 0", instantiate = cold_start})
    zd.append_leaf (leaves, zd.Leaf_Template { name = "= init p.ok = maxhp p.wallet = 0 p.pwr = 4 ndays = 0", instantiate = cold_start})
}
